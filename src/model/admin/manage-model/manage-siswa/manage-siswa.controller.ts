...
export class ManageSiswaController {
    private manageKelasService: ManageSiswaService
    constructor() {
        this.manageKelasService = new ManageSiswaService()
    }
    ...
    public getAllSiswa = async (req: AuthRequest, res: Response, next: NextFunction) => {
        try {
            ...
            const siswa = await this.manageKelasService.fetchAllSiswa()
            console.log(chalk.greenBright("[Backend Controller] Authorized admin user. Received request to get all Kelas data."), siswa)
            res.status(200).json({
    ...
    public getSiswaById = async (req: AuthRequest, res: Response, next: NextFunction) => {
        try {
            ...
            if(req.user.role !== "admin") {
                console.log(chalk.redBright("Forbidden: Only admin users can access Kelas data by ID."))
                throw new ErrorOutput("Forbidden: Only admin users can access Kelas data by ID.", 403)
            }

            const id = Number(req.params.id)
            const siswa_id = await this.manageKelasService.fetchSiswaById(id)
            console.log(chalk.greenBright("[Backend Controller] Authorized admin user. Received request to get Kelas data by ID:"), siswa_id)
            res.status(200).json({ 
