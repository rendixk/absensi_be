...
export class SiswaProfileController {
    ...
    public SiswaProfile = async (req: SiswaAuthRequest, res: Response, next: NextFunction) => {
        try {
            ...
            data_siswa.avatar = `${protocol}://${host}/avatar/${data_siswa.avatar}`

            if(data_siswa.avatar) {
                (data_siswa as any).avatar_url = `${protocol}://${host}/uploads/image/siswa_pfp/${data_siswa.avatar}`
            }

            console.log(chalk.greenBright("[Backend Controller] Authorized siswa. Profile fetched."))
    ...
    public editSiswaProfileController = async (req: SiswaAuthRequest, res: Response, next: NextFunction) => {
        try {
            ...
            const protocol = req.protocol
            const host = req.get("host")
            if(siswa_profile_update.avatar) {
                (siswa_profile_update as any) = `${protocol}://${host}/uploads/image/siswa_pfp/${siswa_profile_update.avatar}`
            }
            console.log(chalk.blue(`[Backend Controller] Image URL generated: ${host}/uploads/image/siswa_pfp/${data_siswa.avatar}`))
