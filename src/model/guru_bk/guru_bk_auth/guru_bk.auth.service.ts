...
export const guru_bk_auth_register_service = async (data: { username: string, password: string, nip: string }) => {
    const exist_guru_bk = await find_guru_bk_username_repo(data.username)
    if(exist_guru_bk) {
        console.log(chalk.redBright("Guru BK with this username already exists."))
...
export const guru_bk_auth_login_service = async (username: string, raw_password: string) => {
    ...
    const token = generate_token({ id: guru_bk_account.id, username: guru_bk_account.username, role: guru_bk_account.role })
